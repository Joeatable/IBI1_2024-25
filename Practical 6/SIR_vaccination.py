...
n = float(input(" vaccination rate (%)  =  " ))/100 # vaccination rate
N = 10000 # total population
I = int(input(" infected people  =  " )) # initial number of infected individuals
R = 0 # initial number of recovered individuals
V = int(round(N * n ))# initial number of vaccinated individuals
...
for i in range(1000): # simulate the time step
    if S == 0:  # insure that there exist the source of infection and people will be infected.
        ...
        infection_prob = ( beta * I ) / ( N - V ) # the rate of infection is changed!! it is determined as (beta*I)/(N-V)
        infection_prob = np.clip (infection_prob,0,1)  # ensure the prob is between 0 and 1
        new_I_individuals = np.random.choice([0, 1], size=int(round(S)), p=[1 - infection_prob, infection_prob]).sum() 
        # use np.random.choice to randomly select individuals to be infected,0 -> safe; 1 -> infected
        new_R_individuals = np.random.choice([0, 1], size=int(round(I)), p=[1 - gamma, gamma]).sum()
...
plt.title('SIR_vaccination Model') # define title
plt.legend() 
plt.savefig("/Users/joey/Desktop/IBI_2024-25/IBI1_2024-25/Practical 6/SIR_vaccination10.png") #save them in my file 
